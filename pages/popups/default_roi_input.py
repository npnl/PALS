...
class DefaultROIInputPopup(Toplevel, object):
	def __init__(self, controller):
		...
		options_harvard = [ NameVarStore(controller, 'Label_%d'%x) for x in range(15)]
		ch_list_harvard = CheckboxList(self, controller, option_heading, options_harvard, row=1, column=0)
		...
		ch_list_freesurfer = CheckboxList(self, controller, option_heading, options_freesurfer, row=1, column=1)
		...
		ch_list_freesurfer_sub = CheckboxList(self, controller, option_heading, options_freesurfer_sub, row=2, column=0, columnspan=2)
