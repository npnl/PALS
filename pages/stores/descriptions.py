class Descriptions(object):
	def __init__(self):
		...
		self.brain_ext = "Indicate if you have already performed brain extraction for all subjects. Each subject directory should contain a skull-stripped brain with your skull-stripped brain identifier. If not, PALS will perform brain extraction for every subject using FSL BET. NOTE: Skull-stripped brain files must be present in each subject directory. If any subject is missing a brain file, PALS will run brain extraction on all subjects."

		self.bet_identifier = "If you have performed brain extraction for all subjects, indicate the unique identifier for skull-stripped brain files. For example, 'Brain' if subject1's brain file is subj01_Brain.nii.gz. NOTE: Skull-stripped brain files must be present in each subject directory. If any subject is missing a brain file, PALS will run brain extraction on all subjects."
		...
		self.lf_subject_file = "This is for QC only. Once a QC page has been generated by PALS, a link to the page will show up. Use the link to visually inspect the outputs and select subjects that do not pass inspection. At this point, you will be able to download a textfile containing the subjects you flagged. If you would like to remove these subjects from analysis, import the downloaded textfile into the space below. If you would like to keep all subjects in the analysis pipeline, click on 'Continue with all subjects'."
